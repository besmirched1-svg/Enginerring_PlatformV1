...
def _compression_roller_mass(cfg: dict | None, material: str) -> float:
    """Solid cylindrical nip roller. Volume = pi * (D/2)^2 * width."""
    cfg = cfg or {}
    D = float(cfg.get("diameter", 200)) / 1000.0
    W = float(cfg.get("width",   4000)) / 1000.0
    return _mass_kg(math.pi * (D / 2.0) ** 2 * W, material)
...
MASS_CALCULATORS = {
    ...
    "Drum":              _drum_mass,
    "CompressionRoller": _compression_roller_mass,
    "Roller":            _roller_mass,
    "Hopper":            _hopper_mass,
}
